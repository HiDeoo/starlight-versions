import path from 'node:path'

import { describe, expect, test, vi } from 'vitest'

import { addPrefixToSidebarConfig, getDocSlug, getURLSlug } from '../libs/starlight'

describe('getDocSlug', () => {
  const docsDir = new URL('content/docs/', import.meta.url)

  test('handles root index file', () => {
    expect(getDocSlug(docsDir, new URL('index.md', docsDir))).toBe('/')
  })

  test('handles nested index files', () => {
    expect(getDocSlug(docsDir, new URL('guides/index.md', docsDir))).toBe('guides')
  })

  test('handles root files', () => {
    expect(getDocSlug(docsDir, new URL('test.mdx', docsDir))).toBe('test')
  })

  test('handles nested files', () => {
    expect(getDocSlug(docsDir, new URL('guides/examples/test.mdx', docsDir))).toBe('guides/examples/test')
  })

  test('handles Windows-style nested files', () => {
    const relativeSpy = vi.spyOn(path, 'relative').mockReturnValue(String.raw`guides\examples\test.mdx`)

    try {
      expect(getDocSlug(docsDir, new URL('guides/examples/test.mdx', docsDir))).toBe('guides/examples/test')
    } finally {
      relativeSpy.mockRestore()
    }
  })

  test('handles Windows-style nested index files', () => {
    const relativeSpy = vi.spyOn(path, 'relative').mockReturnValue(String.raw`guides\examples\index.mdx`)

    try {
      expect(getDocSlug(docsDir, new URL('guides/examples/index.mdx', docsDir))).toBe('guides/examples')
    } finally {
      relativeSpy.mockRestore()
    }
  })
})

describe('addPrefixToSidebarConfig', () => {
  test('does not prefix absolute links', () => {
    expect(addPrefixToSidebarConfig('5.0', [{ label: 'Test', link: 'https://starlight.astro.build' }]))
      .toMatchInlineSnapshot(`
      [
        {
          "label": "Test",
          "link": "https://starlight.astro.build",
        },
      ]
    `)
  })

  test('prefix links to the home page', () => {
    expect(addPrefixToSidebarConfig('4.0', ['', 'index'])).toMatchInlineSnapshot(`
      [
        "4.0",
        "4.0",
      ]
    `)
  })

  test('prefix root links', () => {
    expect(
      addPrefixToSidebarConfig('3.0', [
        { label: 'Test 1', link: '/test-1/' },
        { label: 'Test 2', link: './test-2/' },
        { label: 'Test 3', link: '/' },
        { label: 'Test 4', link: 'test-4' },
      ]),
    ).toMatchInlineSnapshot(`
      [
        {
          "label": "Test 1",
          "link": "/3.0/test-1/",
        },
        {
          "label": "Test 2",
          "link": "./3.0/test-2/",
        },
        {
          "label": "Test 3",
          "link": "/3.0/",
        },
        {
          "label": "Test 4",
          "link": "3.0/test-4",
        },
      ]
    `)
  })

  test('prefix links in groups', () => {
    expect(
      addPrefixToSidebarConfig('3.0', [
        {
          label: 'Group',
          items: [
            { label: 'Test 1', link: '/group/test-1/' },
            { label: 'Test 2', link: './group/test-2/' },
            { label: 'Test 3', link: 'group/test-3' },
          ],
        },
      ]),
    ).toMatchInlineSnapshot(`
      [
        {
          "items": [
            {
              "label": "Test 1",
              "link": "/3.0/group/test-1/",
            },
            {
              "label": "Test 2",
              "link": "./3.0/group/test-2/",
            },
            {
              "label": "Test 3",
              "link": "3.0/group/test-3",
            },
          ],
          "label": "Group",
        },
      ]
    `)
  })

  test('does not prefix manual sidebar entries for excluded pages', () => {
    expect(
      addPrefixToSidebarConfig(
        '3.0',
        ['test', { slug: 'guides/test' }, { label: 'Test', link: './test/?foo=bar#baz' }, 'versioned'],
        ['test', 'guides/test'],
      ),
    ).toEqual(['test', { slug: 'guides/test' }, { label: 'Test', link: './test/?foo=bar#baz' }, '3.0/versioned'])
  })

  test('prefixes sidebar entries that are not excluded', () => {
    expect(addPrefixToSidebarConfig('3.0', ['guides/test'], ['fr/guides/test'])).toEqual(['3.0/guides/test'])
  })

  test('migrates and prefixes legacy autogenerated groups', () => {
    expect(
      addPrefixToSidebarConfig('2.0', [
        {
          label: 'Group',
          autogenerate: { directory: 'test' },
        },
      ]),
    ).toMatchInlineSnapshot(`
      [
        {
          "items": [
            {
              "autogenerate": {
                "directory": "2.0/test",
              },
            },
          ],
          "label": "Group",
        },
      ]
    `)
  })

  test('preserves legacy autogenerated group collapsed inheritance', () => {
    expect(
      addPrefixToSidebarConfig('2.0', [
        {
          label: 'Group',
          collapsed: true,
          autogenerate: { directory: 'test' },
        },
      ]),
    ).toMatchInlineSnapshot(`
    [
      {
        "collapsed": true,
        "items": [
          {
            "autogenerate": {
              "collapsed": true,
              "directory": "2.0/test",
            },
          },
        ],
        "label": "Group",
      },
    ]
  `)
  })

  test('preserves explicit legacy autogenerated collapsed value', () => {
    expect(
      addPrefixToSidebarConfig('2.0', [
        {
          label: 'Group',
          collapsed: true,
          autogenerate: { directory: 'test', collapsed: false },
        },
      ]),
    ).toMatchInlineSnapshot(`
    [
      {
        "collapsed": true,
        "items": [
          {
            "autogenerate": {
              "collapsed": false,
              "directory": "2.0/test",
            },
          },
        ],
        "label": "Group",
      },
    ]
  `)
  })

  test('prefixes autogenerated entries', () => {
    expect(
      addPrefixToSidebarConfig('2.0', [
        {
          label: 'Group',
          items: [{ autogenerate: { directory: 'test' } }],
        },
      ]),
    ).toMatchInlineSnapshot(`
    [
      {
        "items": [
          {
            "autogenerate": {
              "directory": "2.0/test",
            },
          },
        ],
        "label": "Group",
      },
    ]
  `)
  })

  test('prefix links in groups', () => {
    expect(
      addPrefixToSidebarConfig('3.0', [
        { slug: 'test-root-1' },
        'test-root-2',
        {
          label: 'Group',
          items: [
            { label: 'Test 1', slug: 'group/test-nested-1' },
            { slug: 'group/test-nested-2' },
            'group/test-nested-3',
          ],
        },
      ]),
    ).toMatchInlineSnapshot(`
      [
        {
          "slug": "3.0/test-root-1",
        },
        "3.0/test-root-2",
        {
          "items": [
            {
              "label": "Test 1",
              "slug": "3.0/group/test-nested-1",
            },
            {
              "slug": "3.0/group/test-nested-2",
            },
            "3.0/group/test-nested-3",
          ],
          "label": "Group",
        },
      ]
    `)
  })
})

describe('getURLSlug', () => {
  test('does not strip a partial base segment', () => {
    vi.stubEnv('BASE_URL', '/docs')

    expect(getURLSlug(new URL('https://example.com/docs-old/guides/test/'))).toBe('docs-old/guides/test')

    vi.unstubAllEnvs()
  })
})
